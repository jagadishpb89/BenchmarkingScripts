...
if len(sys.argv) <= 2:
  ...
  print commands.getoutput("mkdir -p symmetry-pods-yamls")
  for index in range(int(sys.argv[1])):
     f = open("template-symmetry-pod-rs.yaml", "r")
     filedata = f.read()
     f.close()
     ...
     filedata = filedata.replace("nginx-symmetry-pod", "nginx-symmetry--pod-" + str(index) + "-" + str(int(sys.argv[7])))
     filedata = filedata.replace("replicas_value", sys.argv[2])
     newfile = "symmetry-pods-rs-" + str(index) + "-" + str(int(sys.argv[7])) + ".yaml"
     f_new = open("symmetry-pods-yamls/" + newfile, "w")
     f_new.write(filedata)
     f_new.close()
     cmd = "kubectl create -f " + "symmetry-pods-yamls/" + newfile + " &"
     print cmd   
     os.system(cmd)
